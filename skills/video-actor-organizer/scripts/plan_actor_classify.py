...
def parse_args():
    ...
    parser.add_argument("--root", required=True, help="扫描根目录")
    parser.add_argument(
        "--target-root",
        default=None,
        help="目标根目录（默认与 --root 相同，用于避免扫描目录和目标目录相同时出现嵌套）",
    )
    parser.add_argument(
...
def find_first_nfo(folder: Path) -> Path | None:
    ...
    """
    try:
        nfo_files = [item for item in folder.iterdir() 
        ...
        for nfo in nfo_files:
            name_lower = nfo.stem.lower()
            if '.cd' not in name_lower and '-cd' not in name_lower:
                return nfo
        
        # 其次找 cd1
        for nfo in nfo_files:
            name_lower = nfo.stem.lower()
            if '.cd1' in name_lower or '-cd1' in name_lower or name_lower.endswith('cd1'):
                return nfo
...
def scan_folders(
    ...
    for folder in video_folders:
        ...
        # 默认跳过已分类目录下的文件夹
        if not include_organized and is_in_classified_path(
        ...
        if is_vr:
            if (
                ...
                dest_folder = (
                    target_root / big_dir / category / initial / display_name / title
                )
            else:
                dest_folder = target_root / big_dir / category / display_name / title
        else:
            if (
                classification["type"] in ("chinese", "chinese_mapped")
                and initial
                ...
                dest_folder = target_root / category / initial / display_name / title
            else:
                dest_folder = target_root / category / display_name / title

        try:
            if dest_folder.resolve() == folder.resolve():
                continue
        except OSError:
            pass
...
def main():
    ...
    # 目标根目录：优先使用 --target-root，否则使用 --root
    target_root = Path(args.target_root).resolve() if args.target_root else root_path
    if not target_root.exists():
        print(f"Error: Target root directory does not exist: {target_root}")
        exit(1)
    ...
    print(f"Include organized: {args.include_organized}")
